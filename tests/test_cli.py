...
class TestCLI(unittest.TestCase):
    ...
    def test_cli_file_not_found(self, mock_print, mock_exit):
        ...
        print_args = [call.args for call in mock_print.call_args_list]
        self.assertTrue(
            any("not found" in args[0] for args in print_args if len(args) > 0)
        )

    @patch("sys.exit")
    @patch("builtins.print")
    def test_cli_invalid_workers(self, mock_print, mock_exit):
        ...
        self.assertTrue(
            any(
                "must be positive" in call.args[0] for call in mock_print.call_args_list
            )
        )
    ...
    @patch("sys.exit")
    @patch("builtins.print")
    def test_cli_workers_type_validation(self, mock_print, mock_exit):
